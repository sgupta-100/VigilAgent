...
class GeminiClient:
    ...
    async def generate_embedding(self, text: str, scan_ctx=None) -> List[float]:
        """
        Generate a vector embedding via text-embedding-004.
        ...
        url = f"{GEMINI_API_URL}/models/{GEMINI_EMBEDDING_MODEL}:embedContent?key={self._api_key}"
        ...
        try:
            async with self._session.post(
                ...
                if response.status == 200:
                    ...
                    logger.info(f"GEMINI: Embedding generated (dim={len(values)})")
                    return values
